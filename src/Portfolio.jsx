...
const aiProjects = [
  {
    ...
    image:"/images/valur.png", imageAlt:"Valur logo, where the VOC pipeline was built",
  ...
  },
  {
    ...
    tags:["AI","Competitive Intelligence","Automation","Web Scraping"],
    oneLiner:"A system that watches your competitors so you don't have to. It runs on a schedule, compares what changed, and only alerts you when something actually matters.",
    image:null,
    metrics:[{v:"5",l:"Competitor URLs tracked"},{v:"Weekly",l:"Automated scan cycle"},{v:"<2 min",l:"Per competitor analysis"},{v:"Zero",l:"Manual monitoring needed"}],
    problem:"Competitive intelligence in most startups is reactive. Someone notices a competitor launched something, mentions it in Slack, and then marketing scrambles to figure out what changed and whether it matters. There's no system. No historical record. No way to distinguish a meaningful positioning shift from a cosmetic homepage tweak. At best, someone bookmarks five competitor pages and checks them when they remember. At worst, **you find out about a competitor's new messaging from your prospect during a sales call**.",
    research:"Mapped the competitive monitoring workflows at multiple startups and identified the core failure pattern: **the bottleneck isn't analysis, it's consistent observation**. Teams are capable of responding to competitive shifts, but they don't catch them fast enough because no one is systematically watching. Evaluated existing CI tools (Klue, Crayon, Kompyte) and identified that most are overkill for early-stage teams. The need was for a lightweight, autonomous system that does one thing well: watch, compare, and surface what matters.",
    solution:"Built an **autonomous monitoring system** that runs on a weekly schedule with zero human input required between cycles. **Step 1: Fetch.** A Python script with BeautifulSoup scrapes 5 defined competitor URLs (homepages, pricing pages, product pages) and stores the current version. **Step 2: Compare.** Claude API receives the current and previous versions and generates a structured diff: what messaging changed, what new features were announced, what positioning shifted, what pricing moved. **Step 3: Evaluate.** Claude scores each change on a materiality scale. Cosmetic tweaks get logged silently. **Significant shifts trigger an alert** with a summary and a recommended response. **Step 4: Alert.** Material changes push to Slack or email with a digestible brief. Everything gets logged in a Google Sheet as a running competitive record. The system is **fully autonomous**: once configured, it runs without being asked, stays quiet when nothing matters, and speaks up when something does.",
    results:"The system identified **messaging repositioning, new feature announcements, and pricing changes** across tracked competitors within the first scan cycle. Weekly digests replaced ad-hoc Slack threads. The historical log enabled trend analysis: instead of reacting to individual changes, the team could see **patterns in competitor behavior over time**. Sales started using the competitive briefs in deal prep. Total setup time: under a day. Running cost: negligible. **Built with:** Python, BeautifulSoup, Claude API (Anthropic), Google Sheets API, Slack Webhook."
  },
  {
